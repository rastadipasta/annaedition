  ...
  return (
    <>
      ...
      <article className="container project-detail-hero">
        ...
        <div className="project-hero-image" data-motion="load" data-motion-variant="image" data-motion-order="3"><Image src={project.cover} alt={`${project.title} – Hauptansicht`} fill priority sizes="100vw" /></div>
        <div className="project-story"><div data-motion="reveal"><p className="eyebrow">Materialwelt</p><ul className="material-list">{project.materials.map((material) => <li key={material}>{material}</li>)}</ul></div><div data-motion="reveal" data-motion-order="1"><h2 className="display" style={{ fontSize: "clamp(3rem, 6vw, 6rem)", marginTop: 0 }}>Ein Raum mit <span className="accent">eigener Geschichte.</span></h2><p className="lede">{project.description}</p></div></div>
        <div className="gallery">{project.gallery.map((image, imageIndex) => <figure key={`${image}-${imageIndex}`} data-motion="reveal" data-motion-variant="image" data-motion-order={String(imageIndex % 3)}><Image src={image} alt={`${project.title} – Ansicht ${imageIndex + 1}`} fill sizes="(max-width: 800px) 100vw, 50vw" /></figure>)}</div>
